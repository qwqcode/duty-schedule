  ...
  <div class="page fullscreen builder-page">
    ...
    <div class="inner">
      ...
      <div class="step step-1" v-if="stepNum === 1">
        <GrpList :asSelector="true" v-model="grpSelList"></GrpList>
      </div>
      </slide-x-left-transition>
      ...
      <div class="step step-3 build-done" v-if="stepNum === 3">
        ...
        <div class="action-bar">
          <el-button-group>
            <router-link :to="{ name: 'viewer', params: { title: !!plan ? plan.name : '' }}"><el-button size="mini">点击展示</el-button></router-link>
    ...
    <div class="step-toggle-bar" v-if="stepNum < Object.keys(stepDesc).length">
      <div class="left" v-if="stepNum === 1">
        <template v-if="stepNum === 1">已选取 {{ grpSelList.length }} 个小组</template>
      </div>
...
export default class Builder extends Vue {
  ...
  nextStep () {
    // 切换 step 前
    if (this.stepNum === 1 && this.grpSelList.length === 0) {
      window.notify('请选择小组')
      return
    }
